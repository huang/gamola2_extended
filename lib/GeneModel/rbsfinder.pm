...
#RBSfinder as PERL module
#return RBSoutput as reference
#writes RBSoutput as filename.rbsfinder

package GeneModel::rbsfinder;
use strict;
...
#local variables
my (%args, $key, $value, $msg, $manual, $RBSoutput, $RBS_region_len, $SD_seq,
    $gene_coord_file_ref, $start_coord, $end_coord, $orf_name, $RBS_region,
    $RBS, $max_RBS_score, $max_RBS_loc, $max_RBS_start, $max_RBS, $RBS_up,
    $RBS_loc_up, $RBS_score_up, $how_far_to_move, $new_start_coord,
    $RBS_loc, $RBS_score, $seq_ref, @COORD_FILE,
    $original_start_codon, $new_start_upstream, $new_start_coord_up,
    $tmp_threshold, $seq, $markov_score_threshold, %orf_previous, $upstreamreloc,
    @SD_markov_score, $gene_coord_file, @ORF_LIST, @ORFLIST, @RBS_seq, $max_seq,
    %loc_hash, $max_loc, $tmp_score, $max_score, $perfect_markov_score,
    $upstream, %is_operon);
...
#############################################################################
# Copyright (c) 2000, 2001 The Institute for Genomic Research
# Code written by Baris Ethem Suzek suzek@cs.jhu.edu
#    2/26/2001
#    rbs_finder.pl
#
# Usage:
...
#
#      where:
#           whole_dna_sequence: the file containing all the sequence
...
#############################################################################
#
# code modifed, fixed and adapted for GAMOLA in 2006
#
##############################################################################

   %args = @_;

   ${$args{progress_bar}}->configure(-label=>"Refining gene model with RBSfinder");
   ...
   &progress_bar_2(main_window  => $args{main_window},
                   ...
                   title        => "Calculating Ribosomal Binding Sites for $args{filename}",
                   label        => ''
                  );
   &show_pbar_2;
   ...
   open OUTPUT, '>', \$RBSoutput;
   $RBS_region_len=${$args{ini_ref}}{RBSlength};
   $SD_seq=${$args{ini_ref}}{RBSseq};

   #remove empty lines in gene coordinates
   ${$args{gene_coord_ref}} =~ s/^\n$//g;
   ...
   #****************************
   @COORD_FILE = split/\n/,${$args{gene_coord_ref}};
   my $number_of_ORFs = scalar @COORD_FILE;
   my $progress = 1;
   foreach (@COORD_FILE){
      #indicate progress
      &update_pbar_2(title        => "Calculating Ribosomal Binding Sites for $args{filename}",
                     label        => "Analysing genes",
                     progress     => ($progress / $number_of_ORFs) * 100,
                    );
      $progress++;
      ...
      if ($start_coord<$end_coord){
          ...
          while($new_start_coord<$how_far_to_move){
            ...
            if ($RBS eq "xxxxxx") {$RBS = ""};
            if (($RBS_score>$max_RBS_score) && ($RBS ne "")){
                $max_RBS=$RBS;
          ...
          if  (($new_start_upstream  ne "" )&&($RBS_score_up>$max_RBS_score)){
            ...
            if ($max_RBS ne ""){
                ...
                printf(OUTPUT "%10s      %8d    %8d    %8s %8s        %5s         %5d   %5s %8d\n",
                  $orf_name,
                  $start_coord,
                  $end_coord,
                  "---",
...
sub next_start_coord_fw
{
   ...
   while(1){
      ...
      elsif ($start_codon eq "gtg"){
         #if ($o_start ne "atg"){
         return $tmp_start;
         #}
      }
      elsif ($start_codon eq "ttg"){
         #if (($o_start ne "atg") && ($o_start ne "gtg")){
         return $tmp_start;
         #}
      }
   }
...
sub next_start_coord_reverse
{
   ...
   while(1){
      ...
      if ($start_codon eq "cat"){
         return $tmp_start;
      }
      elsif ($start_codon eq "cac"){
         #if ($o_start ne "atg"){
         return $tmp_start;
          #}
      }
      elsif ($start_codon eq "caa"){
         #if (($o_start ne "atg") && ($o_start ne "gtg")){
         return $tmp_start;
          #}
      }
   ...
   if ($start_coord<$end_coord){ #the orf processed is in forward strain
      if ($orf_previous{$orf_name}>0){ #if previous orf is in same direction
         ...
         for ($y=$start_coord-4;$y>$orf_previous{$orf_name};$y-=3){
            ...
            if (($codon eq "atg")
                || ((($codon eq "gtg") && ($o_start ne "atg"))
                || (($codon eq "ttg") && (($o_start ne "atg") && ($o_start ne "gtg"))))){
                ...
                if ($tmp_RBS eq "xxxxxx") {$tmp_RBS = ""};
                if ($tmp_RBS ne ""){
                   @tmp_RBS_arr=split(" ",$tmp_RBS);
                   if ($tmp_RBS_arr[2]>$tmp_max_RBS_score){
                       $tmp_max_RBS=$tmp_RBS;
...
sub calculate_markov_score
{
    ...
    for (my $r=0;$r<length($tmp_seq);$r++){
       if (exists($SD_markov_score[$r]{substr($tmp_seq,$r,1)})) {
...
sub train_SD_markov
{
   ...
   #to use in 0th order Markov model
    ...
    for (my $q=0;$q<length($SD_seq);$q++){
       unless (exists($SD_markov_score[$q]{"a"})) {
         $SD_markov_score[$q]{"a"} = "0";
       }
       if ($SD_markov_score[$q]{"a"}+0.000001 <= 0) {next};
       $SD_markov_score[$q]{"a"}=log(($SD_markov_score[$q]{"a"}+0.000001)/$number_of_sample);
       unless (exists($SD_markov_score[$q]{"t"})) {
         $SD_markov_score[$q]{"t"} = "0";
       }
       if ($SD_markov_score[$q]{"t"}+0.000001 <= 0) {next};
       $SD_markov_score[$q]{"t"}=log(($SD_markov_score[$q]{"t"}+0.000001)/$number_of_sample);
       unless (exists($SD_markov_score[$q]{"g"})) {
         $SD_markov_score[$q]{"g"} = "0";
       }
       if ($SD_markov_score[$q]{"g"}+0.000001 <= 0) {next};
       $SD_markov_score[$q]{"g"}=log(($SD_markov_score[$q]{"g"}+0.000001)/$number_of_sample);
       unless (exists($SD_markov_score[$q]{"c"})) {
         $SD_markov_score[$q]{"c"} = "0";
       }
       if ($SD_markov_score[$q]{"c"}+0.000001 <= 0) {next};
       $SD_markov_score[$q]{"c"}=log(($SD_markov_score[$q]{"c"}+0.000001)/$number_of_sample);
    }

    for(my $key=0;$key<=($RBS_region_len-length($SD_seq));$key++){
       unless (exists($loc_hash{$key})) {
         $loc_hash{$key} = "0";
       }
       if ($loc_hash{$key} < 0) {
          $loc_hash{$key} = 0;
