package Basics::Rename;

use strict;
BEGIN { eval { require warnings; warnings->import } }

package Basics::Rename::Options;
...
sub rename (\@$;$) {
	...
	if( 'HASH' eq ref $code ) {
	    ...
	    if(defined $verbose ) {
 		Carp::carp(<<CARP);
Basics::Rename::rename: third argument ($verbose) ignored
CARP
	    } 
	    $verbose = $code;
	    $code = delete $verbose->{_code}
	  	or Carp::carp(<<CARP);
Basics::Rename::rename: no _code in $verbose
CARP
...
Basics::Rename - Perl extension for renaming multiple files

=head1 SYNOPSIS

  use Basics::Rename qw(rename);		# hide CORE::rename
  rename @ARGV, sub { s/\.pl\z/.pm/ }, 1;

  use Basics::Rename;
  Basics::Rename::rename @ARGV, '$_ = lc';
