...
static void 
getlineRSV(struct ReadSeqVars *V)
{
...
readPIR(struct ReadSeqVars *V)
{
  ...
  getlineRSV(V);			/* skip next line, coords */
...
readNBRF(struct ReadSeqVars *V)
{
  ...
  getlineRSV(V);   /*skip title-junk line*/
...
readZuker(struct ReadSeqVars *V)
{
  ...
  getlineRSV(V);  /*s == "seqLen seqid string..."*/
...
readSquid(struct ReadSeqVars *V)
{
  ...
  while (strncmp(V->sbuffer, "NAM ", 4) != 0) getlineRSV(V);
